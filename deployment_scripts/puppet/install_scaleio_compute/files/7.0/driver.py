...
libvirt_volume_drivers = [
    ...
    'scaleio=nova.virt.libvirt.volume.LibvirtScaleIOVolumeDriver',
]
...
class LibvirtDriver(driver.ComputeDriver):
    ...
    def attach_volume(self, context, connection_info, instance, mountpoint,
        ...
        disk_info = blockinfo.get_info_from_bdm(CONF.libvirt.virt_type,
                                                image_meta, bdm)
        LOG.info(_LI("disk_info %s"),disk_info)
	LOG.info(_LI("connection info %s"),connection_info)

	self._connect_volume(connection_info, disk_info)
        conf = self._get_volume_config(connection_info, disk_info)
        self._set_cache_mode(conf)
        LOG.info(_LI("Magdy conf driver info %s"),conf.to_xml())
