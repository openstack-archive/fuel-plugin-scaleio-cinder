...
class ScaleIODriver(driver.VolumeDriver):
    ...
    def create_volume(self, volume):
        ...
        if (domain_id == None):
            ...
            if (r.status_code != OK_STATUS_CODE and "errorCode" in domain_id):
                msg = (
                "Error getting domain id from name %s: %s " % (self.protection_domain_name, domain_id['message']))
                LOG.error(msg)
                raise exception.VolumeBackendAPIException(data=msg)
        ...
        params = {'protectionDomainId': domain_id, 'volumeSizeInKb': str(volume_size_kb), 'name': volname,
        ...
        r = requests.post("https://" + self.server_ip + ":" + self.server_port + "/api/types/Volume/instances",
                          data=json.dumps(params), headers=headers, auth=(self.server_username, self.server_token),
                          verify=verify_cert)
        response = r.json()
    ...
    def _snapshot_volume(self, volname, snapname):
        ...
        r = requests.post(request, data=json.dumps(params), headers=headers,
                          auth=(self.server_username, self.server_token), verify=verify_cert)
        r = self._check_response(r, request)
        response = r.json()
    ...
    def _check_response(self, response, request):
        if (response.status_code == 401 or response.status_code == 403):
            ...
            res = requests.get(request, auth=(self.server_username, self.server_token), verify=verify_cert)
            return res
        return response
    ...
    def create_volume_from_snapshot(self, volume, snapshot):
        ...
        LOG.info("ScaleIO create volume from snapshot: snapshot {0} to volume {1}".format(volname, snapname))
        self._snapshot_volume(volname, snapname)
    ...
    def extend_volume(self, volume, new_size):
        ...
        if (r.status_code != OK_STATUS_CODE):
            response = r.json()
            msg = ("Error extending volume %s: %s" % (volname, response['message']))
            LOG.error(msg)
            raise exception.VolumeBackendAPIException(data=msg)
    ...
    def _delete_volume(self, volname):
        ...
        if (r.status_code != OK_STATUS_CODE and "errorCode" in vol_id):
            ...
            if (error_code == VOLUME_NOT_FOUND_ERROR):
                ...
                if (force_delete == 'True'):
                    msg = (
                    "Ignoring error in delete volume %s: volume not found due to force delete settings" % (volname))
                    LOG.warning(msg)
                    return
        ...
        r = requests.post("https://" + self.server_ip + ":" + self.server_port + "/api/instances/Volume::" + str(
            vol_id) + "/action/removeVolume", data=json.dumps(params), headers=headers,
                          auth=(self.server_username, self.server_token), verify=verify_cert)
        r = self._check_response(r, request)
        ...
        if (r.status_code != OK_STATUS_CODE):
            ...
            if (error_code == 78):
                ...
                if (force_delete == 'True'):
                    msg = (
                    "Ignoring error in delete volume %s: volume not found due to force delete settings" % (vol_id))
                    LOG.warning(msg)
                else:
    ...
    def _update_volume_stats(self):
        ...
        for sp_name in self.storage_pools:
            ...
            LOG.info("ScaleIO get domain id by name request: %s" % request)
            LOG.info(
                "username: %s, password: %s, verify_cert: %s " % (self.server_username, self.server_token, verify_cert))
            r = requests.get(request, auth=(self.server_username, self.server_token), verify=verify_cert)
            r = self._check_response(r, request)
            LOG.info("Get domain by name response: %s" % r.text)
            domain_id = r.json()
            ...
            for res in response.values():
                ...
                LOG.info("free capacity of pool {0} is: {1}, total capacity: {2}".format(pool_name, free_capacity_gb,
                                                                                         total_capacity_gb))
        ...
        while not disk_filename:
            if (tries > 15):
                raise exception.VolumeBackendAPIException(
                    "scaleIO volume {0} not found at expected path ".format(volume_id))
            by_id_path = "/dev/disk/by-id"
            if not os.path.isdir(by_id_path):
                LOG.warn("scaleIO volume {0} not yet found (no directory /dev/disk/by-id yet). Try number: {1} ".format(
                    volume_id, tries))
                tries = tries + 1
                time.sleep(1)
                continue
    ...
    def copy_image_to_volume(self, context, volume, image_service, image_id):
        ...
        LOG.info("ScaleIO copy_image_to_volume volume: " + str(volume) + " image service: " + str(
            image_service) + " image id: " + str(image_id))
    ...
    def copy_volume_to_image(self, context, volume, image_service, image_meta):
        ...
        LOG.info("ScaleIO copy_volume_to_image volume: " + str(volume) + " image service: " + str(
            image_service) + " image meta: " + str(image_meta))
        properties = utils.brick_get_connector_properties()
        sdc_ip = properties['ip']
        LOG.debug("SDC ip is: {0}".format(sdc_ip))
        try:
